let keyPattern = []; // Will be set by user input
let currentKeyIndex = 0;
let intervalId = null;
let intervalMs = 1000; // Default interval
...
window.onload = function() {
    ...
    function simulateKeyPress() {
            let keyCode = keyPattern[currentKeyIndex];
            if (keyCodes.hasOwnProperty(keyCode)) {
                ...
                // Make the move
                myGame.move(keyCodes[keyCode]);

	    // Highlight next move
                updateHighlight();
            }
            currentKeyIndex++;
	    if (currentKeyIndex >= keyPattern.length) {
		    currentKeyIndex = 0;
	    }
    }

	 // Update highlight for the next move
    function updateHighlight() {
	    let cki = currentKeyIndex;
	    if (intervalMs < 200) {
		cki = -1;
	    }
        const sequenceDisplay = document.getElementById('sequence-display');
        const sequenceItems = sequenceDisplay.textContent.split(', ');
        sequenceDisplay.innerHTML = sequenceItems.map((item, index) =>
            index === cki ? `<span class="highlight">${item}</span>` : item
        ).join(', ');
    }

    // Event listener for the start game button
    document.getElementById('start-game-button').addEventListener('click', function() {
        let keySequenceInput = document.getElementById('key-sequence').value;
    ...
    intervalMs = parseInt(document.getElementById('interval').value, 10);
        ...
        currentKeyIndex = 0;
        updateHighlight(); // Highlight the first move
        intervalId = setInterval(simulateKeyPress, intervalMs); // Use the set interval
	    if (tryAgainButton.style.display != "") {
        myGame.start();
        tryAgainButton.style.display = "";
	    }
    });
